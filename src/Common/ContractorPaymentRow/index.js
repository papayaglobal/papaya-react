...
const ContractorExpandedPaymentRow = props => {
  ...
  const result = [
    ...
    >
      ...
      <Flex column flex={12}>
        <StyledSubmittedText>
          <strong>You</strong>'ve submitted a payment request for the{" "}
          {dateRange} payment period
        </StyledSubmittedText>
        <StyledAttachmentTitle margin={"25px 0 5px 0 "}>
          Pro Forma Invoice
        </StyledAttachmentTitle>
  ...
  if (isRejected) {
    ...
    result.push(
      <Flex
        key={"payment-expanded-item-rejected"}
        row
        className={"expanded-container-rejected"}
        margin={"0 0 20px 0"}
      >
        <Flex column flex={2} padding={"5px 0"}>
        ...
        <Flex column flex={12}>
          <StyledSubmittedText>
            <strong>{updatedBy}</strong> rejected your payment request.
          </StyledSubmittedText>
  ...
  if (isApproved) {
    ...
    result.push(
      <Flex
        key={"payment-expanded-item-approved"}
        row
        className={"expanded-container-approved"}
        margin={"0 0 20px 0"}
      >
        <Flex column flex={2} padding={"5px 0"}>
          <CreatedDate>{createdAtDate}</CreatedDate>
        ...
        <Flex column flex={12}>
          <StyledSubmittedText>
            <strong>{updatedBy}</strong> approved the payment request.
          </StyledSubmittedText>
...
const ContractorPaymentRowExpandedContainer = props => {
  ...
  return (
    <StyledExpandedContainer column isExpanded={isExpanded}>
      {map(payments, (payment, key) => (
        <ContractorExpandedPaymentRow
          key={key}
          payment={payment}
          isLastPaymentRequest={payments.length === key + 1}
          {...otherProps}
        />
      ))}
    </StyledExpandedContainer>
...
const findProFormaByType = ({ proFormaFiles, type }) =>
  find(proFormaFiles, proForma => get(proForma, "file.type") === type);
...
class ContractorPaymentRow extends Component {
  ...
  onInvoiceClicked = ({ e, payment, contractorPaymentRequestInvoice }) => {
    ...
    isFunction(onInvoiceClicked) &&
      onInvoiceClicked({ payment, contractorPaymentRequestInvoice });
  };
      ...
      <StyledPaymentContainer>
        ...
        >
          <StyledLeftWrapper className="leftWrapper">
            ...
            />
            <StyledDates className={"date-range"} isMonthly={true}>
              {dateRange}
            </StyledDates>
            {!isExpanded && (
              <>
                {value && (
                  <StyledAmount justifyStart flex={1} className="amountWrapper">
                ...
                {proforma && (
                  <StyledAttachment className="attachments">
                    <ListItem
                      hideClose={true}
                      onClick={() =>
                        this.onProFormaClicked({
                          payment,
                          contractorPaymentRequestProForma: proforma
                        })
                      }
          ...
          {!isExpanded && (
            <StyledRightWrapper className="rightWrapper">
              {proforma && (
                <StyledAttachmentIcon className="attachments">
                  <AttachmentIcon
                    attachments={[proforma]}
                    onClick={e =>
                      this.onProFormaClicked({
                        e,
                        payment,
              ...
              {!!contractorPaymentRequestInvoice && (
                <StyledAttachments className="attachments md">
                  <AttachmentIcon
                    attachments={[contractorPaymentRequestInvoice]}
                  />
                </StyledAttachments>
              )}
              {!!createdAtAsText && (
                <StyledReportedDate className="reportedDateWrapper">
                  {createdAtAsText}
                </StyledReportedDate>
              )}
              {actions && (
                <StyledActions className="moreWrapper">
                  <Dropdown
                    list={actions}
                    icon={more}
        ...
        <ContractorPaymentRowExpandedContainer
