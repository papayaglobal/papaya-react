...
export const fullDateResponsive = window.matchMedia("(max-width: 767px)")
  .matches
  ? "MMM D, YYYY"
  : "MMMM D, YYYY";
...
export const formatDateRange = props => {
  const { startedAt, endedAt, format = fullDateResponsive, compact = true } =
    props || {};
  ...
  if (compact) {
    ...
    if (moment(startString).isSame(endedAt, "month")) {
      ...
      if (format === fullDateResponsive) {
        const fullDatePart1 = moment(startString).format(
          _(format)
            .replace(/Y/g, "")
            .replace(/,/g, "")
            .trim()
        );
        const fullDatePart2 = moment(endString).format(
          _(format)
            .replace(/M/g, "")
            .trim()
        );
        ...
        return `${moment(startedAt).format(
          _(format)
            .replace(/Y/g, "")
            .replace(/M/g, "")
            .trim()
    ...
    if (moment(startString).isSame(endedAt, "year")) {
      if (format === fullDateResponsive) {
        return `${moment(startString).format(
          _(format)
            .replace(/Y/g, "")
            .replace(/,/g, "")
            .trim()
